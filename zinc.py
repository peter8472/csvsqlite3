'find and order nutrients by sinc content'
import pdb
import sys
import csv
import sqlite3
...
def query3(filename, serving_size = 15.0, nutrient="sodkium"):
    t1 = Timer()
    ...
    cur.execute("select description,amount from temp order by cast (amount  as float) desc;")
    
    rows = cur.fetchall()
    for i in rows:
        print(i) 
    cur.execute( "select * from nutrient " )
...
#    maker= csvsqlite3.tablemaker( "newdb")
#   maker.save_to_database( "../nutrient/nutrient.csv",tablename='nutrient')
    
   
    query3("newdb",  serving_size=14, nutrient='Zinc%')
